...
$total_column = 4;
$report_name = 'Individual JV for FV Allocation';
  ...
  <body>
		...
		<div id="printArea">
		    ...
		    $bundle_type = $this->customcache->bundle_maker($bundleValue,'TYPE');
  			...
  			$down_payment = $this->customcache->bundle_maker($bundleValue,'DOWN_PAYMENT');
					...
					foreach($get_bundle_item as $kEMIITEM=>$vEMIITEM){
						if( $vEMIITEM->BUNDLE_ID == $bundleValue ){
							...
							for($i=1; $i<=$emi_month; $i++){
								...
								if($down_payment){
									$revenue += $temp_revenue + ($down_payment / $emi_month * $fv_allocation / 100);
								}else{
				...
				foreach($get_sale as $k=>$v){
					...
					if( in_array($vs->SERVICE_ID, $bundle_service_id[$bundle_id]) ){
						...
						if( $bundle_type != 'emi' ){
							...
							if($vs->TYPE == 'loop'){
								if($sale_month == $report_month){
									...
									if( $vs->LOOP_DAYS < $calculate_day ){
										$calculate_day = $vs->LOOP_DAYS;
									}
							
									if( $last_of_month > $service_end_date ){
								...
								$earned_value += $one_day_value * $calculate_day;
								
							}elseif( $sale_month == $report_month ){
								$earned_value += ($temp_break_price * $v->QUANTITY);
							}*/
						...
						if( $vs->VAT_APPLICABLE=='yes' && $sale_month==$report_month ){
							...
							#echo $temp_service_price.' - '.$total_sd.' - '.$total_vat.'<br />';
						}
			...
			<table class="table" border="1" cellpadding="0" cellspacing="0" style="width:auto;" align="center">
				...
				<tr>
					...
					<td class="text-center" align="center"><?php echo round($earned_revenue[$vs1->SERVICE_ID],2); ?></td>
				</tr>
				<?php endforeach; ?>
				...
				<tr>
					...
					<td>SD</td>
					<td class="text-center" align="center">&nbsp;</td>
					<td class="text-center" align="center"><?php echo round($total_sd,2); ?></td>
				</tr>
				...
				<tr>
					...
					<td>VAT</td>
					<td class="text-center" align="center">&nbsp;</td>
					<td class="text-center" align="center"><?php echo round($total_vat,2); ?></td>
				</tr>
				...
				<tr>
					...
					<td class="text-center" align="center">&nbsp;</td>
					<td class="text-center" align="center"><?php echo round($service_interest,2); ?></td>
				</tr>
