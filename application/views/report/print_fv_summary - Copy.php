...
$unique_service_id = $this->webspice->unique_multidim_array($get_bundle_item, 'SERVICE_ID');
			
$url_prefix = $this->webspice->settings()->site_url_prefix;
$site_url = $this->webspice->settings()->site_url;
$domain_name = $this->webspice->settings()->domain_name;
$total_column = count($unique_service_id)+5;
		...
		<div id="printArea">
			...
			<table class="table table-bordered table-striped" border="1" align="center">
				...
				echo '<tr>';
					...
					foreach($unique_service_id as $k=>$v){
						echo '<th align="center" class="text-center">'.$this->customcache->service_maker($v,'SERVICE_NAME').'<br />'.$this->customcache->service_maker($v,'GL_CODE').'</th>';
					}
		    ...
		    $bundle_service_info = array();
				foreach($unique_service_id as $k1=>$v1){
			    foreach( $get_bundle_item as $itemKey1=>$itemValue1 ){
						if( $itemValue1->SERVICE_ID == $v1 ){
			    		$bundle_service_info[$itemValue1->BUNDLE_ID][$itemValue1->SERVICE_ID]['QUANTITY'] = $itemValue1->QUANTITY;
			    		$bundle_service_info[$itemValue1->BUNDLE_ID][$itemValue1->SERVICE_ID]['UNIT_PRICE'] = $itemValue1->UNIT_PRICE;
			    		$bundle_service_info[$itemValue1->BUNDLE_ID][$itemValue1->SERVICE_ID]['LOOP_DAYS'] = $itemValue1->LOOP_DAYS;
			    		$bundle_service_info[$itemValue1->BUNDLE_ID][$itemValue1->SERVICE_ID]['TYPE'] = $itemValue1->TYPE;
			    		$bundle_service_info[$itemValue1->BUNDLE_ID][$itemValue1->SERVICE_ID]['VAT_APPLICABLE'] = $itemValue1->VAT_APPLICABLE;
			    	}
			    }
				}
			    ...
			    foreach( $get_bundle_item as $itemKey=>$itemValue ){
			    	if( $itemValue->BUNDLE_ID == $bundleValue ){
				    	...
				    	$bundle_service_id[] = $itemValue->SERVICE_ID;
				    }
			    }
    			...
    			if($bundle_type=='emi'){
						...
						foreach($get_bundle_item as $kEMIITEM=>$vEMIITEM){
							if( $vEMIITEM->BUNDLE_ID == $bundleValue ){
								...
								$emi_service_total[$vEMIITEM->SERVICE_ID] = $revenue;
							}
						}
    			}
					...
					echo '<tr>';
						...
						foreach($unique_service_id as $serviceKey=>$serviceValue){
							$service_price = 0;
							...
							foreach($get_bundle_sale as $saleKey=>$saleValue){
								...
								if( $saleValue->BUNDLE_ID == $bundleValue && in_array($serviceValue, $bundle_service_id) ){
									...
									if($temp_service_price){
										if( $temp_service_info['VAT_APPLICABLE'] == 'yes' ){
											$service_sd = ($temp_service_price * $this->webspice->settings()->sd)/100;
											$temp_net_service_sd = $temp_service_price + $service_sd;
				...
				echo '<tr>';
					...
					foreach($unique_service_id as $k22=>$v22){
