...
$report_name = 'Revenue Deferment Summery Report';
...
<html lang="en">
  ...
  <body>
		...
		<div id="printArea" style="overflow:auto;">
			...
			", array($this->webspice->settings()->gl_device))->result();
			?>
		    ...
		    $bundle_mrp = $this->customcache->bundle_maker($uniqueBundleValue,'MRP');
				...
				# get bundle item (without device GL)
		  	$sql_bundle_item = "
		  	SELECT TBL_BUNDLE_ITEM.*,
		  	...
		  	AND TBL_BUNDLE_SERVICE.GL_CODE != ?
		  	";
		  	$get_bundle_item = $this->db->query($sql_bundle_item, array($uniqueBundleValue, $this->webspice->settings()->gl_device))->result();
		    ...
		    $get_bundle_sale = $this->db->query($sql_bundle_sale, array($uniqueBundleValue))->result();
				...
				<div class="text-center"><h3><?php echo ucwords($this->customcache->bundle_maker($uniqueBundleValue,'MATERIAL_NAME')); ?></h3></div>
			...
			<div style="overflow:auto;">
				...
				<table class="table table-bordered table-striped" align="center" style="width:auto;">
					<tr>
						...
						<?php for($i=$start_month; $i<($start_month+$month_count); $i++): ?>
							<?php if($temp_month > 12){ $temp_month = 1; $temp_year++; } ?>
							<th class="text-center" align="center" width="80"><?php echo $this->webspice->month_convert($temp_month); ?><br /><?php echo $temp_year; ?></th>
							<?php $temp_month++; ?>
						<?php endfor; ?>
					</tr>
					...
					<tr>
						...
						<?php for($i=$start_month; $i<($start_month+$month_count); $i++): ?>
							...
							?>
							<td align="center">
								<?php if( $grand_sd[$temp_year][$temp_month] ){echo round($grand_sd[$temp_year][$temp_month],2);} ?>
								</th>
							<?php $temp_month++; ?>
						<?php endfor; ?>
					</tr>
					...
					<tr>
						...
						<?php for($i=$start_month; $i<($start_month+$month_count); $i++): ?>
							...
							?>
							<td align="center">
								<?php if( $grand_vat[$temp_year][$temp_month] ){ echo round($grand_vat[$temp_year][$temp_month],2);} ?>
								</th>
							<?php $temp_month++; ?>
						<?php endfor; ?>
					</tr>
					...
					<tr>
						...
						<?php for($i=$start_month; $i<($start_month+$month_count); $i++): ?>
							...
							<td align="center">
								<?php 
									if(!isset($grand_interest[$temp_year][$temp_month])){$grand_interest[$temp_year][$temp_month] = 0;}
									if( $grand_interest[$temp_year][$temp_month] ){ echo round($grand_interest[$temp_year][$temp_month],2);} 
								?>
								</th>
							<?php $temp_month++; ?>
						<?php endfor; ?>
