...
$total_column = 4;
$report_name = 'Summary JV for FV Allocation';
		...
		<div id="printArea">
			...
			foreach($get_service as $ks=>$vs){
				...
				foreach($get_sale as $k=>$v){
					...
					$sale_date = date("Y-m-d", strtotime($v->INVOICE_DATE));
					if( $bundle_type != 'emi' ){
						$service_end_date = $this->webspice->addDate(date("Y-m-d",strtotime($v->INVOICE_DATE)), $vs->LOOP_DAYS-1, 'days');
					...
					# verify date range
					if( ($sale_month!=$report_month AND $sale_date > $first_of_month) || ($sale_month!=$report_month AND $first_of_month > $service_end_date)  ){
						continue;
					}
					...
					if( in_array($vs->SERVICE_ID, $bundle_service_id[$bundle_id]) ){
						...
						if( $bundle_type != 'emi' ){
							...
							$temp_service_price = $temp_break_price * $v->QUANTITY;
							
							$earned_value += $temp_service_price;

						}elseif( $bundle_type == 'emi' ){
							$temp_service_price = $emi_service_total[$bundle_id][$vs->SERVICE_ID] * $v->QUANTITY;
							$earned_value += $temp_service_price;
		
							# interest calculate
							$temp_interest = $bundle_actual_interest[$bundle_id];
							$temp_interest_with_qty = ($temp_interest * $v->QUANTITY);
							$service_interest += $temp_interest_with_qty;
						}

						# calculate total sd and vat
						if( $vs->VAT_APPLICABLE=='yes' && $sale_month==$report_month ){
							$net_price = $temp_service_price/118.45*100;
							$temp_sd_for_single_item = ($net_price * $this->webspice->settings()->sd)/100;
							
							$total_sd += $temp_sd_for_single_item;
		
							$temp_net_price_sd = $net_price + $temp_sd_for_single_item; # vat on net value + sd
							
							$total_vat += ($temp_net_price_sd * $this->webspice->settings()->vat)/100;
						}
				...
				# calculate sd and vat for each service
				if( $vs->VAT_APPLICABLE=='yes' ){
					$net_price = $earned_value/118.45*100;
					$temp_sd = ($net_price * $this->webspice->settings()->sd)/100;
					$temp_net_price_sd = $net_price + $temp_sd; # vat on net value + sd
					
					$temp_vat = ($temp_net_price_sd * $this->webspice->settings()->vat)/100;

					$earned_value = $earned_value - ($temp_sd+$temp_vat);
				}
				
				$total_service_interest += $service_interest;
				$earned_revenue[$vs->SERVICE_ID] = $earned_value;
				$grand_total_revenue += $earned_value;
			}
			
			$grand_total_revenue += ($service_interest + $total_sd + $total_vat);
			#dd($earned_revenue);
			?>
			...
			<table class="table" border="1" cellpadding="0" cellspacing="0" style="width:auto;" align="center">
				...
				$is_device = false;
				$device_value = 0;
				foreach($get_service as $ks11=>$vs11){
					if($vs11->GL_CODE==$this->webspice->settings()->gl_device){
						$is_device = true;
						$device_value += $earned_revenue[$vs11->SERVICE_ID];
					}
				}
				?>
				...
				<tr>
					<td style="font-weight:bold;">
						<?php
						if($is_device){
							echo $this->webspice->settings()->gl_device;
						}else{ 
							echo $get_service[0]->GL_CODE;
						}
						?>
					</td>
					<td style="font-weight:bold;">
						<?php 
						if($is_device){
							echo 'Initial Rev - Hand Set';
						}else{
							echo $get_service[0]->SERVICE_NAME;
						}
						?>
					</td>
					<td style="font-weight:bold;" class="text-center" align="center"><?php echo $this->webspice->tk(round($grand_total_revenue,2)); ?></td>
				...
				<tr>
					<td>
						<?php echo $this->webspice->settings()->gl_device; ?>
					</td>
					<td>
						<?php echo 'Initial Rev - Hand Set'; ?>
					</td>
					<td class="text-center" align="center">&nbsp;</td>
					<td class="text-center" align="center"><?php echo $this->webspice->tk(round($device_value,2)); ?></td>
				</tr>
				<?php endif; ?>
				
				<?php foreach($get_service as $ks1=>$vs1): ?>
				<?php if($vs1->GL_CODE==$this->webspice->settings()->gl_device){continue;} ?>
				<tr>
					<td><?php echo $vs1->GL_CODE; ?></td>
					<td><?php echo $vs1->SERVICE_NAME; ?></td>
					<td class="text-center" align="center">&nbsp;</td>
					<td class="text-center" align="center"><?php echo $this->webspice->tk(round($earned_revenue[$vs1->SERVICE_ID],2)); ?></td>
				</tr>
				<?php endforeach; ?>
				...
				<tr>
					...
					<td>SD</td>
					<td class="text-center" align="center">&nbsp;</td>
					<td class="text-center" align="center"><?php echo $this->webspice->tk(round($total_sd,2)); ?></td>
				</tr>
				...
				<tr>
					...
					<td>VAT</td>
					<td class="text-center" align="center">&nbsp;</td>
					<td class="text-center" align="center"><?php echo $this->webspice->tk(round($total_vat,2)); ?></td>
				</tr>
				...
				<tr>
					...
					<td class="text-center" align="center">&nbsp;</td>
					<td class="text-center" align="center"><?php echo $this->webspice->tk(round($total_service_interest,2)); ?></td>
				</tr>
