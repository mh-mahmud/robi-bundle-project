...
#dd(md5(12345678));
		...
		# get config value
		$service_tax_sql = "SELECT * FROM TBL_BUNDLE_OPTION WHERE GROUP_NAME = 'service_tax' ";
		$get_service_tax = $this->db->query($service_tax_sql)->result();
		
		if( $get_service_tax ){
			$_SESSION['service_tax'] = $get_service_tax[0]->OPTION_VALUE;
		}else{
			$_SESSION['service_tax'] = 0;
		}
		
		$data['sale_summery_result'] = null;
		$data['sale_summery_result_gross_value'] = null;
		if( $this->webspice->get_user_id() && $this->webspice->permission_verify('dashboard', true) ){
			# get sale summery
			$data['first_of_month'] = $this->webspice->first_of_month(date("Y"), date("m"));
			$data['last_of_month'] = $this->webspice->last_of_month(date("Y"), date("m"));
			$sale_summery_sql = "
			SELECT TBL_BUNDLE_SALE.BUNDLE_ID, TBL_BUNDLE_SALE.INVOICE_DATE,
			SUM(TBL_BUNDLE_SALE.GROSS_VALUE) AS SUM_GROSS_VALUE,
			SUM(TBL_BUNDLE_SALE.QUANTITY) AS SUM_QUANTITY
			FROM TBL_BUNDLE_SALE
			WHERE TBL_BUNDLE_SALE.INVOICE_DATE BETWEEN TO_DATE(?,'yyyy-mm-dd hh:mi:ss') AND TO_DATE(?,'yyyy-mm-dd hh:mi:ss')
			AND TBL_BUNDLE_SALE.STATUS = 7
			GROUP BY TBL_BUNDLE_SALE.INVOICE_DATE, TBL_BUNDLE_SALE.BUNDLE_ID
			";
	
			$data['sale_summery_result'] = $this->db->query($sale_summery_sql, array($data['first_of_month'], $data['last_of_month']))->result();
			
			
			# get sale summery gross value
			$sale_summery_sql_gross_value = "
			SELECT TBL_BUNDLE_SALE.INVOICE_DATE,
			SUM(TBL_BUNDLE_SALE.GROSS_VALUE) AS SUM_GROSS_VALUE,
			SUM(TBL_BUNDLE_SALE.QUANTITY) AS SUM_GROSS_QUANTITY
			FROM TBL_BUNDLE_SALE
			WHERE TBL_BUNDLE_SALE.INVOICE_DATE BETWEEN TO_DATE(?,'yyyy-mm-dd hh:mi:ss') AND TO_DATE(?,'yyyy-mm-dd hh:mi:ss')
			AND TBL_BUNDLE_SALE.STATUS = 7
			GROUP BY TBL_BUNDLE_SALE.INVOICE_DATE
			";
	
			$data['sale_summery_result_gross_value'] = $this->db->query($sale_summery_sql_gross_value, array($data['first_of_month'], $data['last_of_month']))->result();
	...
	function login(){
		...
		TBL_BUNDLE_ROLE.PERMISSION_NAME 
		FROM TBL_BUNDLE_USER
		LEFT JOIN TBL_BUNDLE_ROLE ON TBL_BUNDLE_ROLE.ROLE_ID=TBL_BUNDLE_USER.ROLE_ID
		WHERE TBL_BUNDLE_USER.USER_EMAIL ='".$input->user_email."'
		AND TBL_BUNDLE_USER.USER_PASSWORD=?",
	...
	function forgot_password(){
		...
		$get_record = $this->db->query("SELECT * FROM TBL_BUNDLE_USER WHERE USER_EMAIL=?", array($input->user_email));
	...
	function change_password($param_user_id=null){		
		...
		$get_user = $this->db->query("SELECT * FROM TBL_BUNDLE_USER WHERE USER_EMAIL=?", array($user_id))->result();
		if( !$get_user ){
			$this->webspice->page_not_found();
			return false;
		}
		...
		# update password
		$update = $this->db->query("UPDATE TBL_BUNDLE_USER SET USER_PASSWORD=?, UPDATED_DATE=TO_DATE(?,'yyyy-mm-dd hh:mi:ss'), USER_PASSWORD_HISTORY=?, STATUS=? WHERE USER_EMAIL=?", array($new_password, $this->webspice->now(), $password_history, $STATUS, $user_id));
		if( !$update ){
			# log
